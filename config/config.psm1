class VC_LTL5 {
    [void]config([string]$WindowsTargetPlatformMinVersion,[bool]$CleanImport){
        $private:Platform=""
        $private:lang=@{}
        ...
        $private:modlang=Import-Module "$PSScriptRoot\lang.psm1" -AsCustomObject
        $modlang."Update-Lang2"($lang)
    ...
    [void]config([string]$WindowsTargetPlatformMinVersion){
        config($WindowsTargetPlatformMinVersion,$false)
    }
}
function Get-VC_LTL5{
    return [VC_LTL5]::new()
}