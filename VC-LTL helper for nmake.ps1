...
function global:Add-VC_LTL{
    ...
    $private:lang=@{}
    $private:modlang=Import-Module "$PSScriptRoot\config\lang.psm1" -AsCustomObject -Force
    $modlang."Update-Lang1"($lang)
    if(Test-Path env:HAVE_VC_LTL -PathType Leaf){
        Write-Error $lang.ERROR_VC_LTL_GOT_IT
        return
    }
    while($true)
    {
        ...
        try{
            ...
            Write-Error $lang.ERROR_VC_LTL_CANNOT_FOUND_VC_LTL
            return
        }
    }
    $private:config=Import-Module "$env:VC_LTL_Root\config\config.psm1" -AsCustomObject -Force
    $private:vc_ltl5=$config."Get-VC_LTL5"()
    $vc_ltl5.config($WindowsTargetPlatformMinVersion,$CleanImport)
}
$Reg1 = [regex] "^#.*"
